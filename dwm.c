...
static Clr **scheme;
static Clr **screen_symbol_scheme;
static Display *dpy;
static Drw *drw;
...
cleanup(void)
{
	...
	for (i = 0; i < LENGTH(colors); i++)
		free(scheme[i]);
	for (i = 0; i < LENGTH(screen_symbol_colors); i++)
		free(screen_symbol_scheme[i]);
	free(scheme);
	free(screen_symbol_scheme);
...
drawbar(Monitor *m)
{
	...
	for (i = 0; i < LENGTH(tags); i++) {
		...
		if (occ & 1 << i)
			drw_rect(drw, x + boxs + horizpadbar/2, boxs + vertpadbar/2, boxw, boxw,
	...
	if ((w = m->ww - tw - x) > bh) {
		if (m->sel) {
			...
			drw_text(drw, x, 0, w - 2 * m->gappx, bh, lrpad / 2, m->sel->name, 0);
			if (m->sel->isfloating)
...
setup(void)
{
	...
	screen_symbol_scheme = ecalloc(LENGTH(screen_symbol_colors), sizeof(Clr *));
	for (i = 0; i < LENGTH(screen_symbol_colors); i++)
		screen_symbol_scheme[i] = drw_scm_create(drw, screen_symbol_colors[i], 3);
	/* init bars */
	updatebars();
	updatestatus();
	updatebarpos(selmon);
